...
export default function App() {
  ...
  const members = selectedGroup?.members.map((member) => member.user) ?? [];
  ...
  useEffect(() => {
    if (selectedGroup && members.length) {
      setForm((previous) => ({
        ...
        selected_user_ids: members.map((member) => member.id),
      }));
    }
  ...
  const splitPreview = useMemo(() => {
    ...
    const selectedIds = form.split_mode === "equal_all" ? members.map((member) => member.id) : form.selected_user_ids;
  ...
  async function saveExpense() {
    ...
    setForm((previous) => ({ ...previous, amount_rupees: "", description: "" }));
  ...
  async function parseNaturalLanguage() {
    ...
    if (draft.amount_paise && draft.payer_id && draft.shares.length) {
      setForm({
        ...
        selected_user_ids: draft.shares.flatMap((share) => (share.user_id ? [share.user_id] : [])),
      });
    }
  }
  ...
  function toggleFormMember(userId: number) {
    setForm((previous) => ({
      ...
      selected_user_ids: previous.selected_user_ids.includes(userId)
        ? previous.selected_user_ids.filter((id) => id !== userId)
        ...
        <div className="panel">
          ...
          {parsedExpense && (
            <div className="draft">
              ...
              {parsedExpense.warnings.map((warning) => (
